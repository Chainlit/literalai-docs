...
Additionally, you can pass the optional `batch_size` argument to batch multiple events - handled simultaneously by Literal. Defaults to 10.
...
literal_client = LiteralClient(api_key=os.getenv("LITERAL_API_KEY"), disabled=True)
```
