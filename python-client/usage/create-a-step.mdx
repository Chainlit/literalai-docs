...
step.input = { "content": "test input" }
# do something
step.output = { "content": "Hello world", "additional_key": "foobar" }
step.end()
```
...
<ParamField path="input" type="dict">
  ...
  It is recommended to use the `content` key for displaying a message within the platform.
</ParamField>
