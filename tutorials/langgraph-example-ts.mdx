---
title: Build a LangGraph agent workflow 
---

In this small sample code, we'll explore how to build a LangGraph agent workflow and log its runs on Literal AI using the callback handler.

<Frame caption="A LangGraph run logged in Literal AI">
  <img
    src="/images/langchain_run_ts.png"
    alt="A LangGraph run"
  />
</Frame>
...
async function main() {
  ...
  // Wrap the calls in a Literal AI thread so they are grouped in the UI
  literalClient.thread({ name: "Weather Wrap" }).wrap(async () => {
    ...
    const nextState = await app.invoke(
      ...
      {
        // Because the thread_id is the same, this call will inherit the message history
