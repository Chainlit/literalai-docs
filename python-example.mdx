...
## Logging the conversation with Chainlit

First, we initialize the Chainlit client.

```python
client = ChainlitClient(api_key="")
```
...
To run the example, you need to install the Chainlit client:
...
In the Chainlit platform, you will see the following thread being logged:

<Frame caption="Rendering of the Thread">
  <img src="/images/python-example.png" />
</Frame>