...
* We've adapted the Online Evaluation page for our on-premise clients who may not have a Queuing system installed
* Interactions with filters and layouts on the dashboard are now tracked on PostHog
...
* Some changes on the way the platform is deployed, both on our end and for our on-premise users :
  ...
  * The `BUCKET_NAME` environment variable is no longer variable. Trying to store objects will log errors but not disrupt the rest of the operations
