...
* Experiments:
  * The link between Experiment and Dataset is now optional
  * Experiment items are now sortable by score in a leaderboard fashion
  * Each experiment item now has a pointer to visualize the trace of its corresponding experiment run 
* Generations: you can add metadata to logged LLM calls

### Improvements
* Added explicit Custom provider option for custom endpoints - following OpenAI's messages API
* The champion version in the Prompt versions table is now highlighted with a star
* From the Prompt Playground, you can now copy to clipboard the template messages as an array
* If created via a Score Template, a Score gets linked to its template
* Input column for Generations now displays the last message in OpenAI's messages API
