...
An experiment should be run when you want to evaluate the performance of your prompt/agent.

To run an experiment, you need:
- A prompt or agent to evaluate.
- A set of inputs/expected outputs (usually a [Dataset](/concepts/dataset)).
- A set of metrics to evaluate the output of your prompt/agent against the expected output.
...
Offline Experiments are not running directly on Literal AI, instead they are run on your local machine or on your own infrastructure.

This allow you to run complex experiments with your own code and tools and then log the results on Literal AI.
...
# Run the experiment the same way as before, except that the inputs are coming from the dataset

def run_experiment():
    for item in dataset.items:
        run_and_eval(item.input)
```
```typescript TypeScript
...
Online Experiments are running directly on Literal AI. This allows you to run experiments without having to manage the infrastructure.
