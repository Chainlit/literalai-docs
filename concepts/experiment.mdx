...
Through [`Dataset`](/concepts/dataset) objects, users can iterate on a parameter of the LLM system - such as prompt template, LLM provider, temperature, etc - and view its impact via Experiments. 
...
- on a [`Dataset`](/concepts/dataset) which contains a number of steps' inputs/outputs to score &mdash; these are **offline evaluations**
