...
To add a tag to a Thread or Generation, open the specific item and add the tag on the top of the window. Here, you can also add new tags. Adding tags to Steps is only possible via the [SDK](http://localhost:3000/python-client/api-reference/step#update-a-step) at the moment. 
...
updated_thread = await client.api.update_thread(
    id="<THREAD_UUID>",
    tags=["good run", "positive sentiment"], # add tags here
)
...
const thread = await client.api.upsertThread({ 
  ...
  tags: ["good run", "positive sentiment"]});
```
...
const filtered_threads = await client.api.getThreads({ filters: { tags: ["good run"] }});
```
