...
In the following code example you can see that you can tag steps with the type `"Run"`, by writing `@literal_client.step(type="run")` in Python or `@literal_client.step(type="run")` in TypeScript:
