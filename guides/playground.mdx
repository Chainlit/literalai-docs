...
- `With Code`: Get a code snippet to use the prompt in your application. (TODO:add link to pull a prompt in your code)
- `Experiment on Dataset`: Run experiments against a dataset of your choice. (TODO: add link to experiment on dataset)
- `Save`: Save the current prompt as a new version. (TODO: add link to prompt management)
...
User messages typicall contain variables to embed a user query along with context. 
...
If statements and for-loops are written like the following: `{{#x}} ... {{/x}}`:
- if `x` is a boolean value, the section tag act like a conditional `if`
- when `x` is an array, they act like a `for each` loop: access each element with `{{.}}`
</Info>
...
Once you are satisfied with your "vibes check", you can run experiments on representative datasets. TODO: add link. 

Saving your prompt comes in last. TODO: add link to prompt management
...
TODO: Move these parts to credentials.
