...
const generateText = literalAiClient.instrumentation.vercel.instrument(baseGenerateText);
...
// both the response from the LLM API and the other informations present in the response (latency, token counts, etc...).
const streamText = literalAiClient.instrumentation.vercel.instrument(baseStreamText);
