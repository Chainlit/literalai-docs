...
You can deploy the LiteralAI platform anywhere by provisioning the infrastructure, setting up the environment variables and running the Docker image.
...
- Run the database and redis cache in a private network so that only the container running the LiteralAI platform can access them.
...
## 4. File Storage Configuration [Optional]
